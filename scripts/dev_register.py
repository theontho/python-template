...
def get_gh_identities():
    ...
    try:
        ...
        status_output = subprocess.check_output(["gh", "auth", "status"], stderr=subprocess.STDOUT).decode()
        ...
        for username in set(usernames): # Use set to avoid duplicates if any
            try:
                user_info_raw = subprocess.check_output(["gh", "api", f"users/{username}"], stderr=subprocess.DEVNULL).decode()
...
def register():
    ...
    choice = input(f"Choose an identity to register in .dev_id (1-{len(all_identities)}) [1]: ") or "1"
    try:
